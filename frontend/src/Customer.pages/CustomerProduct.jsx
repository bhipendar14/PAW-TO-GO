...
const Product = () => {
  ...
  const renderStars = (rating) => {
    return [...Array(5)].map((_, i) =>
      i < Math.floor(rating) ? <FaStar key={i} color="#FFD700" /> :
      i < rating ? <FaStarHalfAlt key={i} color="#FFD700" /> :
            ...
            {filteredProducts.length > 0 ? (
              filteredProducts.map((product) => (
                <div key={product.id} className="product-card">
                  ...
                  <button onClick={() => navigate(`/customer/orders`, { state: { product } })}>Add to Cart</button>
                </div>
              ))
            ) : (
